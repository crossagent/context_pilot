...
```bash
# 场景 1：查询经验（写代码前确认）
python .agents/skills/context_pilot/scripts/call_a2a.py --query "请告诉我当前的订单支付流程是怎么样的？"

# 场景 2：记录经验（发现新信息时）
python .agents/skills/context_pilot/scripts/call_a2a.py --query "记录一条新知识：生产环境的数据库连接密码现在通过环境变量 DB_PROD_PWD 注入，不再写在 config.json 里。"

# 场景 3：更新/纠正经验（发现文档与实际不符时）
python .agents/skills/context_pilot/scripts/call_a2a.py --query "更新知识库：之前记录的 '用户注册默认分配基础权限' 是错的，现在的逻辑是默认不分配任何权限，需要验证邮箱后才分配。"
```
